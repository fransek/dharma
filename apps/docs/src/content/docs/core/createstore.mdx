...
- `syncWithStorageOn?` - When to initially synchronize the store's state with the storage. Defaults to "load".
...
<Aside>
  When persisting state it must be serializable. The default serializer is JSON
  which has some limitations. If you need more versatility
  [superjson](https://github.com/flightcontrolhq/superjson) is recommended.
</Aside>
